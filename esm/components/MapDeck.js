...
function templateMount(label, value) {
  return ___EmotionJSX("p", null, ___EmotionJSX("span", null, ___EmotionJSX("strong", null, label), ":\xA0"), ___EmotionJSX("span", null, value));
}
...
function dadosCities(cliente) {
  ...
  }, templateMount('Street', get(cliente, 'STREET')), templateMount('Number', get(cliente, 'NUMBER')), templateMount('City', get(cliente, 'CITY')), templateMount('Post Code', get(cliente, 'POSTCODE')), templateMount('LAT', get(cliente, 'LAT')), templateMount('LON', get(cliente, 'LON')));
}
...
function updateTooltip({
  ...
  if (object) {
    let html = '';
    if (object != null && object.properties['STREET']) html = renderToString(dadosCities(object.properties));else {
...
export default function MapDeck({
  ...
  const updateDeckViewHandle = useCallback(({
    layers
  }) => {
    if (!map || !deck) return;
  ...
  const updateHande = useCallback(async param => {
...
MapDeck.propTypes = {
  points: _pt.arrayOf(_pt.any).isRequired,
  height: _pt.number.isRequired,
  width: _pt.number.isRequired,
  totalRegistros: _pt.number.isRequired,
  hash: _pt.string.isRequired
};